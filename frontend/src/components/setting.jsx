...
function Usersetting() {
  ...
  async function ChangeCurrencyType(data) {
    ...
    if (
      msg.success === true &&
      msg.message === `Currency type is now set to ${currencytype}` &&
      result.status === 200
    ) {
  ...
  return (
    <div style={{ marginTop: "4%", textAlign: "center" }}>
      ...
      <br />
      <br />
      <Link to="password" className="setting-btn">
        Change Account Password
      </Link>
      <br />
      <br />
      <Link to="changename" className="setting-btn">
        Change Display Name
      </Link>
      <br />
      <br />
      <h1 style={{ fontSize: "40px" }}> ⚠️ Danger ZONE </h1>
      <Link className="setting-btn">Backup Data</Link>
      <br />
      <br />
      <Link to="delete" className="setting-btn">
